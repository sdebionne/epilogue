...
var Resource = function(options) {
  ...
  this.associationOptions = {
    autoPopulate: { read: true, list: true },
...
function autoAssociate(resource) {
  ...
  _.forEach(resource.model.associations, function(association) {
    ...
    if (association.associationType === 'HasOne') {
      ...
      resource.subResources[subResourceName] = hasOneResource(Resource, resource, association);
    } else if (association.associationType === 'HasMany') {
      subResourceName =
        association.target.options.name.plural.toLowerCase();
      resource.subResources[subResourceName] = hasManyResource(Resource, resource, association);
    } else if (association.associationType === 'BelongsTo') {
      subResourceName =
        association.target.options.name.singular.toLowerCase();
      resource.subResources[subResourceName] = belongsToResource(Resource, resource, association);
    } else if (association.associationType === 'BelongsToMany') {
     subResourceName =
       association.target.options.name.plural.toLowerCase();
     resource.subResources[subResourceName] = belongsToManyResource(Resource, resource, association);
    }
  });
}
